...
with torch.no_grad():
    for src, tgt in val_loader:
        ...
        for t in range(tgt.size(1)):
            ...
            mse_loss = mse_criterion(predictions[:, t], tgt[:, t])
            total_losses[t] += loss.item() * tgt.size(0)
            total_mses[t] += mse_loss.item() * tgt.size(0)
            counts[t] += tgt.size(0)

# 平均MAE
avg_maes = [total_losses[t] / counts[t] for t in range(4)]

avg_mses = [total_mses[t] / counts[t] for t in range(4)]
...
print(f"Overall MAE: {maes / 4}")

for t, mse in enumerate(avg_mses):
    mses += mse
    print(f"Average MSE at time point {t+1}: {mse}")

print(f"Overall MSE: {mses / 4}")
